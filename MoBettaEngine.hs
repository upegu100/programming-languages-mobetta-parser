...
-- Env is a "dictionary" of (variable, int) pairs. We use this to model storing
-- values in variables. The HM.Map type constructor supports lookup and update
--of variables.

type Env = HM.Map String Integer

-- We need an empty environment at the beginning of a computation when no
--variables have yet been used.
emptyEnv :: Env
emptyEnv = HM.fromList []

-- Declare a "computation" to combine IO and ability to deal with an environment.
-- StateT combines a state type (in this case Env) and an existing monad (IO) to
-- produce a monad in which a computation is essentially an IO computation that can access and modify the state.
...
boolCalc (Reln cOp expr1 expr2) = do
  n1 <- (intCalc expr1)
  n2 <- (intCalc expr2)
  let comp = opLookup cOp copTable
  return $ comp n1 n2
boolCalc (BBin op expr1 expr2) = do
  b1 <- (boolCalc expr1)
  b2 <- (boolCalc expr2)
  let bfunc = opLookup op bbinopTable
  return $ bfunc b1 b2
boolCalc (BUn op expr) = do
  b1 <- (boolCalc expr)
  let bfunc = opLookup op bunopTable
  return $ bfunc b1
